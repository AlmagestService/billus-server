...
public class Store extends BaseTime{
    ...
    @Column(name = "store_account", unique = true, nullable = false, length = 40, columnDefinition = "VARCHAR(40)")
    @Comment("QR 생성용 식별자")
    private String serviceId;

    @Column(name = "store_account", unique = true, nullable = false, length = 20, columnDefinition = "VARCHAR(20)")
    @Comment("매장 계정")
    ...
    @Column(name = "store_name", unique = true, nullable = false, length = 30, columnDefinition = "VARCHAR(30)")
    @Comment("매장 이름")
    private String storeName;

    @Column(name = "price", nullable = false, length = 8, columnDefinition = "VARCHAR(8)")
    @Comment("식대 가격")
    private Integer price;

    @Column(name = "tel", unique = true, nullable = false, length = 8, columnDefinition = "VARCHAR(8)")
    @Comment("매장 전화번호")
    private String tel;

    @Column(name = "email", unique = true, nullable = false, length = 8, columnDefinition = "VARCHAR(8)")
    @Comment("매장 이메일")
     ...
     * 정보를 UserDetails로 변환
     */
    public CustomStoreDetails toCustomStoreDetails() {
        return new CustomStoreDetails(
                this,
